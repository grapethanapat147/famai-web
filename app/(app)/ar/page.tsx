...
export default async function ArPage() {
  ...
  const { data: recRows } = await supabase
    .from("receivable")
    .select("id, kind, payer_finance_id, sale_id, amount_due, amount_paid, due_at, settled_at, balance")
    .order("due_at", { ascending: true, nullsFirst: false });
  const recs = recRows ?? [];
  const saleIds = [...new Set(recs.map((r) => r.sale_id))];

  const [salesRes, customersRes, companiesRes, unitsRes, variantsRes, colorsRes] = await Promise.all([
    saleIds.length
