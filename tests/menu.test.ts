...
describe("MENU config", () => {
  // 20 หน้าจาก index.html v1.15 + 'models' (FAM-1009) + 'acct' (FAM-1102) + 'registration' (FAM-1100) = 23
  // ('assist' ผู้ช่วย AI ถอดออกจาก UI ที่ FAM-1069 — นอก TOR; โค้ด lib/api ยังอยู่ dormant)
  it("has 6 groups and 23 pages", () => {
    expect(MENU).toHaveLength(6);
    expect(ALL_MENU_KEYS).toHaveLength(23);
    expect(new Set(ALL_MENU_KEYS).size).toBe(23); // ไม่มี key ซ้ำ
  });
  ...
  it("admin sees every page (incl. models); tech does not see users", () => {
    ...
    expect(adminKeys).toHaveLength(23);
